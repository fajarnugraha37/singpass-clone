...
- [ ] T001 Create compliance test directory in `apps/backend/tests/compliance`
- [ ] T002 [P] Update shared configuration schema in `packages/shared/src/config.ts`

## Phase 2: Foundational

- [ ] T003 Extend `ClientConfig` interface in `apps/backend/src/core/domain/client_registry.ts`
- [ ] T004 [P] Add `clients` table to Drizzle schema in `apps/backend/src/infra/database/schema.ts`
- [ ] T005 Update `HARDENED_CLIENT_REGISTRY` mock data in `apps/backend/src/infra/adapters/client_registry.ts`
...
- [ ] T006 [P] [US1] Implement `validateScopes` utility in `apps/backend/src/core/auth/validation.ts`
- [ ] T007 [US1] Integrate scope validation in `RegisterParUseCase.execute` in `apps/backend/src/core/use-cases/register-par.ts`
- [ ] T008 [P] [US1] Create unit tests for scope enforcement in `apps/backend/tests/compliance/scopes.test.ts`
...
- [ ] T009 [P] [US2] Implement `validateUrlSafe` utility (IP & HTTPS check) in `apps/backend/src/core/auth/validation.ts`
- [ ] T010 [US2] Integrate URL safety validation in `RegisterParUseCase.execute` in `apps/backend/src/core/use-cases/register-par.ts`
- [ ] T011 [P] [US2] Create unit tests for URL safety enforcement in `apps/backend/tests/compliance/urls.test.ts`
...
- [ ] T012 [US3] Add `isActive` check to `RegisterParUseCase.execute` in `apps/backend/src/core/use-cases/register-par.ts`
- [ ] T013 [US3] Add `isActive` check to Authorization flow in `apps/backend/src/infra/http/authRouter.ts`
- [ ] T014 [US3] Add `isActive` check to Token exchange in `apps/backend/src/core/use-cases/exchange-token.ts` (or appropriate file)
- [ ] T015 [P] [US3] Create unit tests for activation management in `apps/backend/tests/compliance/activation.test.ts`
...
- [ ] T016 [US4] Update `DrizzleClientRegistry` to support new metadata fields in `apps/backend/src/infra/adapters/client_registry.ts`
- [ ] T017 [US4] Implement test account limit warning (soft limit of 5) in user creation logic
- [ ] T018 [P] [US4] Create integration tests for entity association and metadata retrieval in `apps/backend/tests/compliance/metadata.test.ts`
...
- [ ] T019 Final compliance audit run against local mock environment
- [ ] T020 Update technical documentation in `docs/singpass-server/02-pushed-authorization-request.md`
